...
public class Lexique {
	...
	public void genereLemme() {
		...
		for(int i=0; i<saisie.getExtractArray().size(); i++){
			...
			// test sur les petits mots (determinants) et si la chaine est un entier
			if(saisie.getExtractArray().get(i).length() > 3 && !java.lang.Character.isDigit(saisie.getExtractArray().get(i).charAt(0)) || saisie.getExtractArray().get(i).equals("sur")){
    ...
    public String toString()
    {
    	...
    	for(int i = 0; i < lemmes.size(); i++)
    	{
    		if(lemmes.get(i).size() == 1)
    		{
    			chaine = chaine + lemmes.get(i).get(0);
    		}
    		else
    		{
    			if(lemmes.get(i).get(1).length() == 0)
    			{
    				chaine  = chaine + lemmes.get(i).get(0);
    			}
    			else chaine  = chaine + lemmes.get(i).get(1);
    		}
